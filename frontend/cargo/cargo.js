...
async function buscarCargo() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log("Dados do cargo:", cargo);
            ...
            bloquearCampos(false);
            // Não mostrar erro no console para 404
            return;
        } else {
            throw new Error('Erro ao buscar cargo');
        }
    } catch (error) {
        // Só mostrar erro se não for 404
        if (error.message !== 'Cargo não encontrado. Você pode incluir um novo cargo.') {
            console.error(error);
...
function preencherFormulario(cargo) {
    if (!cargo || typeof cargo.idcargo === 'undefined' || typeof cargo.nomecargo === 'undefined') {
        mostrarMensagem('Dados do cargo inválidos ou não encontrados.', 'error');
        return;
    }
...
async function salvarOperacao() {
    ...
    // Verificação extra para evitar NaN
    if (!currentPersonId || isNaN(currentPersonId)) {
        mostrarMensagem('ID do cargo inválido!', 'error');
        return;
    }
